...
namespace App\Http\Controllers\Backend\Dialysis\Report;
...
class ReportController extends Controller
{
    ...
    public function dayWiseStock(Request $request)
    {
        ...
        $data  = DB::table('items as item_table')->where('item_table.category_id', 2)
        ...
        return view('backend.dialysis.report.dayWiseStock', compact('data'));
    }
    public function sellReport(Request $request)
    {
        ...
        return view('backend.dialysis.report.sellReport', compact('data'));
    }

    function stockReport(Request $request)
    {
        $data = InventoryItem::whereHas('item', function ($item) {
            return $item->where('category_id', 2);
        })->latest();
        $data = $data->get();
        if ($request->ajax()) {
        ...
        return view('backend.dialysis.report.stockReport')
    ...
    function stockSummary(Request $request)
    {
        ...
        return view('backend.dialysis.report.stockSummary')
            ->with(['items' =>  Item::where('category_id', 2)->select('id', 'name')->get()])
            ->with(['genericNames' =>  GenericName::select('id', 'name')->get()])
            ->with(['subcategories' =>  Subcategory::where('category_id', 2)->select('id', 'name')->get()]);
    }
    ...
    function stockAlert(Request $request)
    {
        ...
        return view('backend.dialysis.report.stockAlert');
    }
    function expireReport(Request $request)
    {
        ...
        return view('backend.dialysis.report.expireReport');
    }

    public function purchaseReport(Request $request)
    {
        $data = Purchase::where('purchase_type', 'dialysis')->latest()
        ...
        return view('backend.dialysis.report.purchaseReport', compact('status'));
    }
    public function purchaseDiscountReport(Request $request)
    {
        $data = Purchase::where('purchase_type', 'dialysis')->where('discount_amount', '>', 0)->latest()
            ->when($request->start_date, function ($query) use ($request) {
                if ($request->start_date === true) {
                    return $query->whereDate('purchase_date', '>=', date('Y-m-d'));
                } else {
    ...
    public function patientVisit(Request $request)
    {
        ...
        return view('backend.dialysis.report.patientVisit', compact('doctor', 'department', 'history', 'regularVisit', 'reportVisit'));
    }
    ...
    public function sellSummaryStore(Request $request)
    {
        ...
        return view('backend.dialysis.report.sellSummary', compact('data', 'start_date', 'end_date'));
    }
}
