...
class LabTestResultController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($labTest->category == 'Biochemistry' && $labTest->name == 'OGTT') {
            return view('backend.pathology.makeResult.ogtt', compact('data', 'labTest'));
        }
        ...
        // Serology
        if ($labTest->category == 'Serology' &&  $labTest->name == 'Dengue IgG/IgM') {
        ...
        // Micro Biolo
        // Hematology
        if ($labTest->category == 'Hematology' && $labTest->name == 'Peripheral Blood film (PBF)') {
            return view('backend.pathology.makeResult.pbf.pbf', compact('data', 'labTest', 'units'));
        }
        ...
        if ($labTest->category == 'Hematology' && $labTest->name == 'Bleeding Time (BT)') {
            return view('backend.pathology.makeResult.hematology.BT_CT', compact('data', 'labTest'));
        }
        if ($labTest->category == 'Hematology' && $labTest->name == 'Clotting Time (CT)') {
            return view('backend.pathology.makeResult.hematology.ct', compact('data', 'labTest'));
        }
        ...
        if ($labTest->category == 'Stool' &&  $labTest->name == 'Stool-CS') {
            return view('backend.pathology.makeResult.stool.checkGrowthOrNot', compact('data', 'labTest'));
        }
        // end urine
   ...
   //     dd($request);
        try {
            ...
            if ($testName->category == 'Hematology' && $testName->name == 'Peripheral Blood film (PBF)') {
                ...
                for ($i = 0; $i < count($request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['name']); $i++) {
                    $multidimensionalArray[$i] = array(
                        ...
                        'name_bold' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['name_bold'][$i] ?? '0',

                        'result' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['result'][$i] ?? '',
                        'result_bold' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['result_bold'][$i] ?? '0',
            ...
            if (($testName->category == 'Micro Biology' && $testName->name == 'Blood - C/S' && $request->growth == 'Yes')) {
                ...
                $data['comment'] = $request->fixedComment ?? '';
                $data['sens'] = $request->sensitive ?? '';
                $data['sval'] = $request->sval ?? '';
                $data['resis'] = $request->resistant ?? '';
                $data['rval'] = $request->rval ?? '';
                $data['inter'] = $request->intermediate ?? '';
                $data['inval'] = $request->inval ?? '';
            ...
            if (($testName->category == 'Stool' &&  $testName->name == 'Stool-CS' && $request->growth == 'No')) {
                $data['lab_test_id']                    = $request->test_id;
                $data['lab_invoice_test_detail_id']     = $request->lab_invoice_test_detail_id;
                $data['created_by']                     = auth('admin')->user()->id;
                $data['created_date']                   = date('Y-m-d h:i:s');
    ...
    public function show(Request $request)
    {
        ...
        // start Biochemistry
        if ($labTestReport->testName->category == 'Biochemistry' && $labTestReport->testName->name == 'e-GFR (Using 4 variable MDRD equation)') {
            return view('backend.pathology.viewResult.egfr', compact('labTestReport'));
        }
        ...
        // end Serology
        if ($labTestReport->testName->category == 'Hematology' && $labTestReport->testName->name == 'Peripheral Blood film (PBF)') {
            return view('backend.pathology.viewResult.pbf', compact('labTestReport'));
        }
        // start Hematology
        ...
        if ($labTestReport->testName->category == 'Stool' && $labTestReport->testName->name == 'Stool-CS' && $labTestReport->growth_or_not == false) {
            return view('backend.pathology.viewResult.stool.stool-cs-no-growth', compact('labTestReport'));
        }
        // end stool
    ...
    public function printCat(Request $request)
    {
        ...
        if ($request->category == 'Biochemistry' || $request->category == 'Serology' || $request->category == 'Hematology' || $request->category == 'Immunology' || $request->category == 'Urine' || $request->category == 'Stool') {
            $labInvoice = LabInvoice::whereId($request->invoice_id)
                ->with('patient', 'doctor','labTestDetails')
                ->with(['labTestDetails'
                => function ($query) use ($request) {
                        $query->whereStatus('completed')->whereHas('testName', function ($query) use ($request) {
                            ...
                            ->with('testName:id,name,category,specimen')
                            ->with('viewResult');
                    }])->first();
            ...
            $category = $request->category;
            $reffered = optional($labInvoice->reference)->name ?? null;
            if (count($labTestReport) > 0){
                foreach ($labTestReport as $labTestReports){
                    $labTestReports =($labTestReports->viewResult->result);

                    $labTestReports = json_decode($labTestReports);

                    foreach ($labTestReports as  $key=> $item){
                        $refCount =str_word_count($item->reference_value);
                        if ($refCount !== null) {

                            if ($refCount > 15) {
                                if (count($labTestReport) > 0) {
                                    $labTestReport = $labTestReport->chunk(3);
                                    return view('backend.pathology.viewResult.catWise.show', compact('labTestReport', 'category', 'patient', 'reffered', 'invoice_no', 'date', 'doctor'));
                                }
                            } else {
                                if (count($labTestReport) > 0) {
                                    $labTestReport = $labTestReport->chunk(12);
                                    return view('backend.pathology.viewResult.catWise.show', compact('labTestReport', 'category', 'patient', 'reffered', 'invoice_no', 'date', 'doctor'));
                                }
                            }
    ...
    public function printBarCode(LabInvoice $labInvoice)
    {
        ...
        foreach ($labInvoice->labTestDetails as $key => $details) {
            if($details->testName->name == "OGTT"){
                $printData[$details->testName->category . '(' . rand(1, 100) . ')'] = [rand(1, 100) => [
                    'CUS(2Hurs)'=> 'CUS(2Hurs)',
                ]];
                $printData[$details->testName->category . '(' . rand(1, 100) . ')'] = [rand(1, 100) => [
                    'CUS(F)'=> 'CUS(F)',
                ...
                $printData[$details->testName->category . '(' . rand(1, 100) . ')'] = [rand(1, 100) => [
                    'Af 75 gm G'=> 'Af 75 gm G',
                ]];
                $printData[$details->testName->category . '(' . rand(1, 100) . ')'] = [rand(1, 100) => [
                    'FBS'=> 'FBS',
                ]];
            }else if ($details->testName->name == 'CUS (2Hours)' || $details->testName->name == 'CUS (F)' || $details->testName->name == 'Fasting Blood Sugar (FBS)' || $details->testName->name == 'Blood Glucose 2 Hrs. AFB' || $details->testName->name == 'Blood Glucose 2 Hrs. After 75gm Glucose') {
                $printData[$details->testName->category . '(' . rand(1, 100) . ')'] = [$details->testName->tube->name => [
                    $details->testName->short_name => $details->testName->short_name,
                ]];
            }

            else {
                if($details->testName->tube){
    ...
    public function edit(Request $request)
    {
        ...
        $labTestReport = LabTestReport::whereId($request->id)->with('labInvoiceTestDetails.labInvoice', 'patient', 'testName')->first();
        if ($labTestReport->testName->category == 'Hematology' && $labTestReport->testName->name == 'Peripheral Blood film (PBF)') {
            return view('backend.pathology.makeResult.pbf.edit-pbf', compact('data', 'labTest', 'units', 'labTestReport'));
        }
        if ($labTestReport->testName->category == 'Hematology' && $labTestReport->testName->name == 'CBC') {
        ...
        if ($labTestReport->testName->category == 'Hematology' && $labTestReport->testName->name == 'Blood Group') {
            return view('backend.pathology.makeResult.hematology.edit-rhFactor', compact('data', 'labTest', 'units', 'labTestReport'));
        }
        if ($labTestReport->testName->category == 'Hematology' && $labTestReport->testName->name == 'Clotting Time (CT)') {
            return view('backend.pathology.makeResult.hematology.edit-ct', compact('data', 'labTest', 'labTestReport'));
        }
        if ($labTestReport->testName->category == 'Hematology' && $labTestReport->testName->name == 'Bleeding Time (BT)') {
        ...
        if ($labTestReport->testName->category == 'Serology' &&  $labTestReport->testName->name == 'Dengue IgG/IgM') {
    ...
    public function update(Request $request)
    {
       // dd($request);
        ...
        if ($labTestReport->testName->category == 'Biochemistry' && $labTestReport->testName->name == 'e-GFR (Using 4 variable MDRD equation)') {
            ...
            for ($i = 0; $i < count($request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['name']); $i++) {
                $multidimensionalArray[$i] = array(
                    ...
                    'name_bold' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['name_bold'][$i] ?? '',

                    'result' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['result'][$i] ?? '',
                    'result_bold' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['result_bold'][$i] ?? '',
                    'unit' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['unit'][$i] ?? '',
                    'reference_value' => $request->except('_token', '_method', 'lab_invoice_test_detail_id', 'test_id')['reference_value'][$i] ?? '',
                );
            }
        ...
        if ($labTestReport->testName->category == 'Micro Biology' && $labTestReport->testName->name == 'Blood - C/S' && $request->growth == 'Yes') {
            ...
            $labTestReport->update([
                'result' => $result,
                'comment' => $request->fixedComment ?? '',
                'sens' => $request->sensitive ?? '',
                'sval' => $request->sval ?? '',
                'resis' => $request->resistant ?? '',
                'rval' => $request->rval ?? '',
                'inter' => $request->intermediate ?? '',
                'inval' => $request->inval ?? '',
            ]);
            LabInvoiceTestDetails::where('id', $request->lab_invoice_test_detail_id)->update(['status' => 'completed']);
            return view('backend.pathology.viewResult.blood.blood_cs_growth', compact('labTestReport'));
