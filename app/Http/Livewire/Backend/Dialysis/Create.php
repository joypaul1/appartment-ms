...
namespace App\Http\Livewire\Backend\Dialysis;
...
class Create extends Component
{
    ...
    public function mount()
    {
        $this->tax_percent      = 0;
        $this->userId           = 1;
        $this->subcategories    = Subcategory::active()->where('category_id', 2)->select('id', 'name')->get();
        $this->brands           = Brand::active()->select('id', 'name')->get();
    }
    public function render()
    {
        return view('livewire.backend.dialysis.create')
    ...
    {
        return Item::active()->where('category_id', 2)
