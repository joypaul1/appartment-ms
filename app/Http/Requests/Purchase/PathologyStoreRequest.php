...
class PathologyStoreRequest extends FormRequest
{
    ...
    {
        if (!LabTestPurchase::latest()->first()) {
            return 1;
        } else {
            return LabTestPurchase::latest()->first()->invoice_number + 1;
        }
    }
    ...
    public function storeData()
    {
        try {
            ...
            $purchase = LabTestPurchase::create($data); //data created here
            // dd(  $this->item_id);
            // item data
            for ($i = 0; $i < count($this->item_id); $i++) {

                $v = $purchase->purchaseItems()->create([
                ...
                LabTestTube::whereId($this->item_id[$i])->update([
                    'up_before_tax'             => Str::replace(',', '', ($this->up_before_tax[$i] ?? 0)),
                    'tax_id'                    => $this->tax_id[$i] ?? null,
                    'tax_rate'                  => Str::replace(',', '', ($this->tax_rate[$i] ?? 0)),
                    'up_after_tax'              => Str::replace(',', '', ($this->up_after_tax[$i] ?? 0)),
                ...
                if ($existingRecord) {
                    ...
                    DB::table('lab_test_day_wise_stock')->insert([
                        'previous_qty' =>  LabTestItemCount::whereId($this->item_id[$i])->first() ? LabTestItemCount::whereId($this->item_id[$i])->available_qty : 0,
                        'item_id' =>  $this->item_id[$i],
                ...
                LabTestInventoryItem::create([
                    ...
                    'item_id'               => $this->item_id[$i],
                    'date'                  => $this->purchase_date,
                    'expire_date'           => $this->expire_date[$i],
            ...
            if ($this->due_amount == 0) {
                ...
                $cashflowTransition = $purchase->cashflowTransactions()->create([
                    'url'               => "Backend\Pathology\Purchaser\PurchaseController@show,['id' =>" . $purchase->id . "]",
            ...
            $dailyTransition = $purchase->dailyTransactions()->create([
                'url'               => "Backend\Pathology\Purchase\PurchaseController@show,['id' =>" . $purchase->id . "]",
                'description'       => 'Purchase Item',
